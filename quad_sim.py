...
def Multi_Point2Point() -> None:
    ...
    ctrl1 = controller.Controller_PID_Point2Point(
        ...
        quad.get_time,
        quad.set_motor_speeds,
        params=CONTROLLER_1_PARAMETERS,
        quad_identifier="q1",
    )
    ctrl2 = controller.Controller_PID_Point2Point(
        ...
        quad.get_time,
        quad.set_motor_speeds,
        params=CONTROLLER_2_PARAMETERS,
        quad_identifier="q2",
    )
    # Start the threads
    quad.start_thread(dt=QUAD_DYNAMICS_UPDATE, time_scaling=TIME_SCALING)
    ctrl1.start_thread(
        update_rate=CONTROLLER_DYNAMICS_UPDATE, time_scaling=TIME_SCALING
    )
    ctrl2.start_thread(
        update_rate=CONTROLLER_DYNAMICS_UPDATE, time_scaling=TIME_SCALING
    )
...
def Single_Velocity() -> None:
    ...
    quad = quadcopter.Quadcopter(QUADCOPTER)
    gui_object = gui.GUI(quads=QUADCOPTER)
    # ctrl = controller.Controller_PID_Velocity(
    #     quad.get_state,
    #     quad.get_time,
    #     quad.set_motor_speeds,
    #     params=CONTROLLER_PARAMETERS,
    ...
    while run == True:
        for goal in GOALS:
            # ctrl.update_target(goal)
            for i in range(150):
                quad.set_motor_speeds('q1', [8000, 5000, 8000, 5000])
                gui_object.quads["q1"]["position"] = quad.get_position("q1")
                gui_object.quads["q1"]["orientation"] = quad.get_orientation("q1")
                gui_object.update()
    quad.stop_thread()
...
def parse_args() -> None:
    ...
    parser.add_argument(
        "--sim", help="single_p2p, multi_p2p or single_velocity", default="single_p2p"
    )
    parser.add_argument(
        "--time_scale",
        type=float,
